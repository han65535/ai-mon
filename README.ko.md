...
**설치형:** `out/Installer/AI-Mon-0.3.5-x64-ko.msi`를 더블클릭합니다. 현재 사용자 계정의
...
- 0.3.5부터 Codex와 Claude 모두 설치·로그인된 공식 CLI를 통해 **2분마다 계정 한도를 자동 조회**합니다. 이 PC에서 대화하지 않아도 갱신하며, Codex는 VS Code 확장에 포함된 CLI도 지원합니다. Spark 등 모델 전용 한도와 세션 로그는 기본 Codex 그래프에 섞이지 않습니다. **새로고침**으로 더 일찍 조회할 수 있습니다(최소 15초 간격). Claude 터미널 상태줄 연결은 선택 기능입니다. [연결 구조](docs/QUOTAS.md).
